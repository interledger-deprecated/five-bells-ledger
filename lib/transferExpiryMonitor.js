...
const log = require('@ripple/five-bells-shared/services/log')('expiry monitor');
...
function *expireTransfer(transferId) {
  ...
  yield db.transaction(function*(tr) {
    ...
    if (transfer.state !== 'executed' &&
      ...
      // Apply rejection credits
      let accountsToCredit;
      if (transfer.rejection_credits) {
        accountsToCredit =
          yield accountBalances.calculate(tr, transfer.rejection_credits);
      } else {
