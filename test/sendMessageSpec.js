...
const RpcHandler = require('../src/lib/rpcHandler')

describe('RpcHandler.sendMessage', function () {
  logHelper(logger)
  ...
  beforeEach(function * () {
    ...
    this.params = {
      log: logger.create('rpcHandler'),
      uri: require('../src/services/uriManager'),
      validator: require('../src/services/validator'),
      requestingUser: {name: 'alice', is_admin: false},
  ...
  it('returns 201 if the message is valid', function () {
    ...
    return RpcHandler.sendMessage(this.params, message)
  })
  ...
  it('returns 400 if the message is missing "ledger"', function () {
    ...
    return RpcHandler.sendMessage(this.params, message).then(() => {
      throw new Error()
    }).catch((err) => {
      assert.equal(err.code, RpcHandler.websocketErrorCodes.INVALID_MESSAGE)
      assert.equal(err.message, 'Invalid message')
    })
  })
  ...
  it('returns 400 if "from" doesn\'t match the sender (when the sender isn\'t admin)', function () {
    ...
    return RpcHandler.sendMessage(this.params, message).then(() => {
      ...
      assert.equal(err.code, RpcHandler.websocketErrorCodes.UNAUTHORIZED)
      assert.equal(err.message, 'You do not have permission to impersonate this user')
    })
  })
  ...
  it('returns 422 if the message recipient isn\'t listening', function () {
    ...
    return RpcHandler.sendMessage(this.params, message).then(() => {
      ...
      assert.equal(err.code, RpcHandler.websocketErrorCodes.NO_SUBSCRIPTIONS)
      assert.equal(err.message, 'Destination account could not be reached')
    })
  })
  ...
  it('relays a message with "account"', function * () {
    ...
    yield RpcHandler.sendMessage(this.params, message)
    yield timingHelper.sleep(50)
